...
export function Poll() {
  ...
  const pollState = createPollQuery(params.id);
  const voteMutation = createVoteMutation();

  const options = () => {
    const options = pollState.data?.options.sort((a, b) => a.order - b.order);

    if (!options) {
      return [];
    }

    const votes = pollState.data?.votes.reduce(
      (acc, vote) => {
  ...
  const votedOption = () => pollState.data?.votes.find((vote) => vote.userId === user()?.id);
  ...
  return (
    <Switch>
      <Match when={pollState.status === "pending"}>
        <Title>Loading...</Title>

        <div class="flex flex-col items-center justify-center py-24">
          <BiRegularLoaderAlt class="animate-spin w-8 h-8" />

          <span class="mt-4">Loading poll...</span>
        </div>
      </Match>
      <Match when={pollState.status === "error"}>
        <Title>Something went wrong...</Title>

        <div class="flex flex-col items-center justify-center py-24">
          <FaSolidExclamation class="w-8 h-8" />

          <span class="mt-4">Something went wrong</span>
          <span class="mt-2 text-sm text-gray-500">{(pollState.error as Error).message}</span>
        </div>
      </Match>
      <Match when={pollState.data !== undefined}>
        <Title>{pollState.data!.question}</Title>
        <div>
          ...
          <main class="max-w-[400px] mx-auto w-full py-10">
            <h1 class="text-3xl font-medium mb-4">{pollState.data!.question}</h1>
